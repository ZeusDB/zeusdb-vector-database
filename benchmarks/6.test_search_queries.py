...
results_all = index.query(vector=query_vec, filter=None, top_k=5)
for doc_id, score in results_all:
    print(f"{doc_id} (score={score:.4f})")

# Step 6: Query with custom ef_search for high recall
print("\n--- Querying with high ef_search (ef=400) ---")
results_high_ef = index.query(vector=query_vec, filter={"author": "Alice"}, top_k=5, ef_search=400)
for doc_id, score in results_high_ef:
    print(f"{doc_id} (score={score:.4f})")
