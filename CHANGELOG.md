...
- `space="dot"`, the inner product, as a fourth metric. `search()` reports `1 - dot` so that lower stays better, vectors are stored as given rather than normalised, and `index.space` reads the metric back. It cannot be combined with `quantization_config`, which raises `ValueError` at `create()`. Use `cosine` on normalised vectors where a quantized inner product is wanted.
